...
def _stats() -> tuple[dict[str, float], dict[str, tuple[float, int]], dict[str, dict[str, float]], dict[tuple[str, str], float]]:
    ...
    if stratz_path.exists():
        ...
        pos_meta = {row["hero_id"]: (row["score"], row["matches"]) for row in data.get("meta", [])}
...
def recommend(draft: Draft, limit: int = 3, data: str = "manual") -> list[Recommendation]:
    ...
    for hero in heroes.values():
        ...
        pos_score, pos_matches = pos_meta.get(hero.id, (None, 0)); confidence = pos_matches / (pos_matches + 1000) if data in {"stats", "hybrid"} else 1.0
